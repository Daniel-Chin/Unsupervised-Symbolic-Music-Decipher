...
class TransformerPianoDataset(Dataset):
    def __init__(
        self, dir_path: str = TRANSFORMER_PIANO_DATASET_DIR, 
        ...
        for stem_i, stem in enumerate(tqdm(self.stems, 'Load dataset')):
            x: Tensor = torch.load(path.join(dir_path, f'{stem}_x.pt'))
            y: Tensor = torch.load(path.join(dir_path, f'{stem}_y.pt'))
            n_notes, _ = x.shape
